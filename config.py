...
class Config:
    ...
    def __init__(self, env=DEFAULT_ENV):
        ...
        self.openai_api_key = os.environ.get(
            "OPENAI_API_KEY",
            "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",
        )
...
def parse_args(script_name=None):
    ...
    group.add_argument(
        "--dev", action="store_true", help="Run in development environment"
    )
    group.add_argument(
        "--prod", action="store_true", help="Run in production environment"
    )
...
if __name__ == "__main__":
    ...
    print(
        f"Environment: {'Production' if config.mongodb_uri == PROD_MONGODB_URI else 'Development'}"
    )
