...
$PYTHON_CMD scripts/populate_workshops.py --env prod --studio manifest --ai openai 2>&1 | tee -a "$LOG_FILE"
...
$PYTHON_CMD scripts/populate_workshops.py --env prod --studio vins --ai openai 2>&1 | tee -a "$LOG_FILE"
if [ "${PIPESTATUS[0]}" -ne 0 ]; then
...
$PYTHON_CMD scripts/populate_workshops.py --env prod --studio dna --ai openai 2>&1 | tee -a "$LOG_FILE"
if [ "${PIPESTATUS[0]}" -ne 0 ]; then
...
$PYTHON_CMD scripts/populate_workshops.py --env prod --studio danceinn --ai openai 2>&1 | tee -a "$LOG_FILE"
if [ "${PIPESTATUS[0]}" -ne 0 ]; then
