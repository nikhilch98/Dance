...
class TestTimeDetails(unittest.TestCase):
    ...
    def test_time_details_creation(self):
        ...
        details = TimeDetails(
            day=15, month=3, year=2024, start_time="2:00 PM", end_time="4:00 PM"
        )
...
class TestWorkshopProcessor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.artists = [{"artist_id": "test_artist", "artist_name": "Test Artist"}]

        self.processor = WorkshopProcessor(
            self.mock_openai, self.artists, self.mock_mongo
        )
...
class TestStudioProcessor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.processor = StudioProcessor(
            self.mock_openai, self.artists, self.mock_mongo, version=1, position=0
        )
...
def test_get_artists_data(mock_db_manager):
    ...
    expected_artists = [{"artist_id": "test_artist", "artist_name": "Test Artist"}]
    ...
    mock_collection.find.assert_called_once_with({}, {"artist_id": 1, "artist_name": 1})
