...
class BackgroundQRService:
    ...
    async def process_pending_qr_generation(self, batch_size: int = 20) -> Dict[str, Any]:
        ...
        try:
            ...
            for order_doc in orders_to_process:
                try:
                    success = await self._generate_qr_for_order(order_doc)
                    if success:
                        processed_count += 1
                        logger.info(f"Generated QR code for order {order_doc['order_id']}")
                    else:
                        failed_count += 1
                        failed_orders.append(order_doc['order_id'])
                        logger.error(f"Failed to generate QR code for order {order_doc['order_id']}")
                        
                except Exception as e:
                    failed_count += 1
                    failed_orders.append(order_doc['order_id'])
                    logger.error(f"Error generating QR code for order {order_doc['order_id']}: {str(e)}")
